...
public static class WorkGiver_Warden_Convert_JobOnThing
{
    public static void Prefix(Thing t, Pawn pawn, out bool __state)
    {
        ...
        if (pawn2.guest.interactionMode != ConvertThenEnslave.ConvertThenEnslaveMode)
        {
            return;
        }

        if (ConvertThenEnslaveMod.instance.Settings.ReduceResistance && pawn2.guest.resistance > 0f)
        {
            return;
        }
        ...
        {
            pawn2.guest.interactionMode = PrisonerInteractionModeDefOf.Enslave;
            return;
        }
    ...
    public static void Postfix(Thing t, bool __state)
    {
        ...
        pawn2.guest.interactionMode = ConvertThenEnslave.ConvertThenEnslaveMode;
    }
}